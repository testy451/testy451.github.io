...
import { loadAccountDetails, calculateUserBondDetails, calculateUserCBondDetails } from "./slices/AccountSlice";

import { Home, Stake, ChooseBond, Bond, Dashboard, TreasuryDashboard, Presale, ChooseCBond, CBond } from "./views";
...
function App() {
  ...
  const loadAccount = useCallback(
    loadProvider => {
      ...
      cbonds.map(bond => {
        dispatch(calculateUserCBondDetails({ address, bond, provider, networkID: chainID }));
      });
    },
    [connected],
  );
          ...
          <div className={`${path === "/" ? null : classes.content} ${isSmallerScreen && classes.contentShift}`}>
            <Switch>
              ...
              <Route path="/stake">
                {bonds.map(bond => {
                  return (
                    <Route exact key={bond.name} path={`/stake/${bond.name}`}>
                      <Bond bond={bond} />
                    </Route>
                  );
                })}
              ...
              <Route path="/community">
                {cbonds.map(cbond => {
                  return (
                    <Route exact key={cbond.name} path={`/community/${cbond.name}`}>
                      <CBond bond={cbond} />
                    </Route>
                  );
                })}
