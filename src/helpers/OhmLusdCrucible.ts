...
import { bnb_dxtc, lusd } from "../helpers/AllBonds";
...
export const calcAludelDetes = async (networkID: NetworkID, provider: StaticJsonRpcProvider) => {
  const crucibleAddress = addresses[networkID].CRUCIBLE_bnb_dxtc;
  ...
  let ohmLusdPrice = await bnb_dxtc.getBondReservePrice(networkID, provider);
  let ohmLusdContractAddress = bnb_dxtc.getAddressForReserve(networkID).toLowerCase();
