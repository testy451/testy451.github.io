...
import { allCBonds } from "src/helpers/AllBonds";
...
const initialBondArray = allCBonds;
...
function useCSBonds() {
  ...
  const [cbonds, setBonds] = useState<Bond[] | IAllBondData[]>(initialBondArray);

  useEffect(() => {
    let bondDetails: IAllBondData[];
    bondDetails = allCBonds
      .flatMap(bond => {
  ...
  // Debug Log:
  // console.log(bonds);
  return { cbonds, loading: bondLoading };
}

export default useCSBonds;
