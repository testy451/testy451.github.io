...
`;
    ...
    const endBlock = 0; //(await DistributorContract.nextEpochBlock()).toNumber()
    ...
    const circ = 0; // await sohmMainContract.circulatingSupply();
    const stakingRebase = 0; //stakingReward / circ;

    // const sR = stakingReward.toNumber();
    // const cS = circ.toNumber();
    // console.log(sR, cS);
    const fiveDayRate = 0; // Math.pow(1 + stakingRebase, 5 * 3) - 1;
    const stakingAPY = 0; // Math.pow(1 + stakingRebase, 365 * 3) - 1;
    ...
    const circSupply = 0; // totalSupply - bondBalance

    const marketCap = 0; // circSupply * marketPrice

    const Staked = 0; //circ / 1e9 / circSupply * 100
    ...
    const stakingTVL = 0; // marketCap * (Staked / 100)
