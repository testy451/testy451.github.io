...
import { getBalances, calculateUserBondDetails, calculateUserCBondDetails, fetchAccountSuccess } from "./AccountSlice";
...
export const changeApproval = createAsyncThunk(
  ...
  async ({ bond, provider, address, networkID }: IBaseBondAddressAsyncThunk, { dispatch }) => {
    if (!provider) {
      dispatch(error("Please connect your wallet!"));
      return;
    }
    console.log("approvexxxxxxxxxxx");
    const signer = provider.getSigner();
    // const lpTokenContract = new ethers.Contract(addresses[networkID].LPTOKEN_ADDRESS as string, ierc20Abi, signer);
    ...
    // console.log('daiFaiLaunchAllownace+2', bondAllowance);
    // return dispatch(
    //   fetchAccountSuccess({
    //     bond: {
...
export const calcBondDetails = createAsyncThunk(
  ...
  async ({ bond, value, provider, networkID }: ICalcBondDetailsAsyncThunk, { dispatch }): Promise<IBondDetails> => {
    ...
    // const vestingTerm = 3600 * 24; // hardcoded for now
    let bondPrice = 0,
      bondDiscount = 0,
      valuation = 0,
      bondQuote = 0;
    ...
    if (bond.id == 6) {
      bondPrice = await bondContract.getPrice("BNB", "BUSD");
    } else if (bond.id == 7) {
      bondPrice = await bondContract.getPrice("BNB", "BUSD");
    } if (bond.id == 8) {
      bondPrice = await bondContract.getPrice("ETH", "BUSD");
    }

    console.log("Price: ", bondPrice);


    // const terms = await bondContract.terms();
    // console.log("vesting term: ", terms.vestingTerm.toString());
    const maxBondPrice = 0; //await bondContract.maxPayout();
    const debtRatio = 0; // (await bondContract.standardizedDebtRatio()) / Math.pow(10, 9);
    ...
    let purchased = 0; // await bond.getTreasuryBalance(networkID, provider);
    return {
      bond: bond.name,
      bondDiscount,
      debtRatio,
...
export const bondAsset = createAsyncThunk(
  ...
  async ({ value, address, bond, networkID, provider, slippage }: IBondAssetAsyncThunk, { dispatch }) => {
    ...
    try {
      ...
      bondTx = await bondContract.deposit(bond.id, valueInWei);
      dispatch(
...
export const cbondAsset = createAsyncThunk(
  "bonding/cbondAsset",
  async ({ value, address, bond, networkID, provider, slippage }: IBondAssetAsyncThunk, { dispatch }) => {
    const depositorAddress = address;
    const acceptedSlippage = slippage / 100 || 0.005; // 0.5% as default
    ...
    try {
      if (bond.id == 6) {
        bondTx = await bondContract.deposit(bond.getAddressForReserve(networkID), valueInWei, "BNB", "BUSD");
      } else if (bond.id == 7) {
        bondTx = await bondContract.deposit(bond.getAddressForReserve(networkID), valueInWei, "BNB", "BUSD");
      } else if (bond.id == 8) {
        bondTx = await bondContract.deposit(bond.getAddressForReserve(networkID), valueInWei, "ETH", "BUSD");
      }
      ...
      dispatch(calculateUserCBondDetails({ address, bond, networkID, provider }));
    } catch (e: unknown) {
      const rpcError = e as IJsonRPCError;
      if (rpcError.code === -32603 && rpcError.message.indexOf("ds-math-sub-underflow") >= 0) {
        dispatch(
...
export const redeemBond = createAsyncThunk(
  ...
  async ({ address, bond, networkID, provider, autostake }: IRedeemBondAsyncThunk, { dispatch }) => {
    ...
    try {
      ...
      if (autostake == true) {
        redeemTx = await bondContract.withdraw(bond.id, user.amount);
      } else {
        redeemTx = await bondContract.withdraw(bond.id, 0);
      }  
      // console.log("deposit amount: ", user.amount);
      // redeemTx = await bondContract.withdraw(bond.id, user.amount);
...
export const redeemCBond = createAsyncThunk(
  "bonding/redeemCBond",
  async ({ address, bond, networkID, provider, autostake }: IRedeemBondAsyncThunk, { dispatch }) => {
    if (!provider) {
      dispatch(error("Please connect your wallet!"));
    ...
    try {
      ...
      if (autostake == true) {
        ...
        redeemTx = await bondContract.claimReward();
      }  
      // console.log("deposit amount: ", user.amount);
      // redeemTx = await bondContract.withdraw(bond.id, user.amount);
      const pendingTxnType = "redeem_bond_" + bond + (autostake === true ? "_autostake" : "");
