...
import { redeemCBond } from "../../slices/BondSlice";
...
function BondRedeem({ bond }) {
  ...
  async function onRedeem({ autostake }) {
    await dispatch(redeemCBond({ address, bond, networkID: chainID, provider, autostake }));
  }

  const vestingTime = () => {
    // return prettyVestingPeriod(currentBlock, bond.bondMaturationBlock); (new Date()).getTime();
    return prettyVestingPeriod((new Date()).getTime(), bond.bondMaturationBlock);
  };

  const vestingPeriod = () => {
    const vestingBlock = 0; // parseInt(currentBlock) + parseInt(bondingState.vestingTerm);
    console.log('vestingPeriod', vestingBlock);
  ...
  return (
    <Box display="flex" flexDirection="column">
      <Box display="flex" justifyContent="space-around" flexWrap="wrap">
        ...
        >
          {txnButtonText(pendingTransactions, "redeem_bond_" + bond.name + "_autostake", "Claim and Unstake")}
        </Button>
      </Box>
        ...
        <Box className="bond-data">
          <div className="data-row">
            ...
            <Typography className="price-data">
              {isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.interestDue, 4)} CRANK`}
            </Typography>
          </div>
          <div className="data-row">
            <Typography>Your Staked Amount</Typography>
            <Typography className="price-data">
              {isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.pendingPayout, 4)} LP`}
            </Typography>
          </div>
          <div className="data-row">
