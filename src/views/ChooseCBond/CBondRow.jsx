...
export function BondDataCard({ bond }) {
  ...
  console.log("xxxbondPrice: ", bond.bondPrice);
...
export function BondTableData({ bond }) {
  console.log("xxxbondPrice: ", bond);
  // Use BondPrice as indicator of loading.
  const isBondLoading = !bond.tokenPrice ?? true;
  // const isBondLoading = useSelector(state => !state.bonding[bond]?.bondPrice ?? true);
  const currentAprs = useSelector(state => {
    return state.account.aprs;
  });
      ...
      <TableCell align="left">
        <Typography>
          <>
            ...
            {isBondLoading ? <Skeleton width="50px" /> : trim(bond.tokenPrice, 2)}
          </>
        </Typography>
      </TableCell>
      <TableCell align="center">{`${trim(currentAprs[bond.id], 2)}%`}</TableCell>
      <TableCell align="center">{`${trim(bond.totalStaked * bond.tokenPrice, 2)} $`}
        {/* {isBondLoading ? (
          <Skeleton />
        ) : (
          new Intl.NumberFormat("en-US", {
            ...
            maximumFractionDigits: 0,
            minimumFractionDigits: 0,
          }).format(bond.totalStaked)
        )} */}
      </TableCell>
      <TableCell>
        <Link component={NavLink} to={`/community/${bond.name}`}>
